...
const nonce = await this.provider.getTransactionCount(this.address, 'latest');
        ...
        try {
            ...
            if (feeData.maxFeePerGas && feeData.maxPriorityFeePerGas) {
                ...
                log('GAS', `Fee data from provider is problematic or zero. Falling back to 1 Gwei gasPrice.`, Colors.FgYellow, '⚠️');
                txRequest.gasPrice = ethers.parseUnits('1', 'gwei');
            }
        } catch(e) {
...
throw e;
    ...
    async claimAquaFluxTokens(wallet) {
        ...
        try {
            ...
            log('AQUAFLUX', `Claim tokens transaction sent! TX Hash: ${tx.hash}`, Colors.FgYellow, '🚀');
            await tx.wait();
    ...
    async getAquaFluxSignature(wallet, accessToken, proxyAgent, nftType = 0) {
        try {
    ...
    async run() {
        try {
            const { runAquaFlux, aquaFluxMintsPerWallet, runPrimusTip, primusTipCountPerWallet, primusUsernames } = this.operationParams;
...
(async () => {
    ...
    try { proxyUrls = fs.readFileSync('proxy.txt', 'utf8').split('\n').map(line => line.trim()).filter(Boolean); log('CONFIG', `Loaded ${proxyUrls.length} proxy entries.`, Colors.FgCyan, '✅');
} catch (e) { log('WARNING', 'proxy.txt not found.', Colors.FgYellow, '⚠️');
}

    // Assign PRIMUS_TIP_ABI globally, outside of the class but before AccountProcessor is instantiated.
...
const operationParams = { aquaFluxMintsPerWallet: 0, primusTipCountPerWallet: 0, primusUsernames: [] }; 
